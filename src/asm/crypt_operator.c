...
signed char		renew_buffer(signed char b_code_args, int count_arg, signed char type)
{
...
void			crypt_direct(t_asm *assem, t_cont *content, int size_arg)
{
	...
	if (!(name = ft_strsub(content->content, start, ft_strlen(content->content) - start)))
...
signed char		crypt_argument(t_asm *assem, t_cont **content, t_op *op, int count)
{
	signed char		type;
	...
	else if ((*content)->type == INDIRECT_LABEL || (*content)->type == DIRECT_LABEL)
